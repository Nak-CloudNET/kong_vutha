                ...
                <div class="row bold">
                    ...
                    <p class="bold">
						...
						<?php if($inv->type_of_po == 'exp') { ?>
							<br/>
							<?php if($inv->sale_id) { ?>
								<?= lang("customer_invoices"); ?>:								
								<span><?= lang($sales->reference_no); ?></span>
						<?php 	
							}
						} 
						?>
				...
				<div class="table-responsive">
					<table class="table table-bordered table-hover table-striped print-table order-table">
						...
						<tbody>
							<?php
								$r = 1;	
								foreach ($po_expense as $row){
							?>
								<tr>
									<td style="text-align:center; width:40px; vertical-align:middle;"><?= $r; ?></td>
									<td><?= $row->account_code; ?></td>
									<td><?= $row->narrative; ?></td>
									<td style="text-align:right;"><?= $this->erp->formatMoney($row->amount); ?></td>
								</tr>							
							<?php
								$r++;
							}							
							...
							?>	
								...
								<?php if ($Settings->tax2 && $inv->order_tax != 0) { ?>
									<tr>
										<td></td>
										<td colspan="<?= $col; ?>"
											style="text-align:right; font-weight:bold;"><?= $inv->tax_name; ?>
										</td>
										<td style="text-align:right; padding-right:10px; font-weight:bold;"><?= $this->erp->formatMoney($inv->order_tax); ?></td>
									</tr>
								<?php } ?>
								<tr>
									<td></td>
            ...
            <?php if (!$Supplier || !$Customer) { ?>
                <div class="buttons">
                    ...
                    <div class="btn-group btn-group-justified">
						...
						<div class="btn-group">
                            <a href="<?= site_url('purchases/invoice_add_samet/' . $inv->id) ?>"target="_blank" class="tip btn btn-primary" title="<?= lang('Samet') ?>">
                                <i class="fa fa-money"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('Samet') ?></span>
                            </a>
                        </div>
                        <div class="btn-group">
					...
					<div class="btn-group btn-group-justified">
						<div class="btn-group">
								<a href="<?= site_url('purchases/invoice_purchase_chea_kheng/' . $inv->id) ?>" target="_blank" class="tip btn btn-primary">
									<i class="fa fa-file-text-o"></i>
									<span class="hidden-sm hidden-xs"><?= lang('invoice_chea_kheng') ?></span>
								</a>
						</div>
						<div class="btn-group">
								<a href="<?= site_url('purchases/invoice_purchase_receive_kh_chea_kheng/' . $inv->id) ?>" target="_blank" class="tip btn btn-primary">
									<i class="fa fa-file-text-o"></i>
									<span class="hidden-sm hidden-xs"><?= lang('invoice_receive_kh_chea_kheng') ?></span>
								</a>
							</div>
						</div>
					</div>
